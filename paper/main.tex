...
Air-quality forecasts must provide useful lead time while remaining reliable
during pollution episodes and incomplete monitoring. This matters because
particulate exposure is linked to respiratory and cardiovascular harm
\cite{who2021aqg}. Hourly PM2.5 is difficult to forecast because persistence,
calendar cycles, abrupt events, missingness, and instrument changes occur in
the same record.

TimeMixer represents multiscale temporal structure
\cite{wang2024timemixer}, whereas XGBoost models nonlinear interactions among
lagged, rolling, and regime features \cite{chen2016xgboost}. We therefore use
TimeMixer for the base trajectory and XGBoost for horizon-specific residuals.
Median de-biasing, pseudo-Huber loss, bounded corrections, held-out gating,
and rolling calibration protect against unstable residual corrections.

Using 168 origin-available hours, the study forecasts 1, 6, 12, 24, and 48
hours without future weather or traffic inputs. Five UK stations, DLinear,
LSTM, tree and naive baselines, seeds 0--4, and a 30-epoch maximum test whether
the hybrid improves both TimeMixer and the strongest matched comparator.

\section{Related Work}
\label{sec:related}

LSTM remains a standard sequence comparator \cite{hochreiter1997lstm}, while
decomposition-based models show the value of separating temporal modes
\cite{bai2019ensemble}. PatchTST uses local subsequences
\cite{nie2023patchtst}; TimeMixer instead downsamples and mixes multiple
scales. XGBoost complements these sequence models by targeting structured
residual error rather than relearning the full trajectory.

Because accuracy changes with lead time \cite{beijing2021forecast}, results
are reported by horizon using chronological, common-origin comparisons. MAE is
primary \cite{willmott2005mae}; paired inference accounts for serial
dependence \cite{diebold1995accuracy,newey1987hac}. All reported values are
generated from saved experiment outputs.

\section{From Historical PM2.5 Time Series to Multi-Horizon Residual Correction}
\label{sec:framework}

The pipeline separates raw evidence from modeling values, short-gap
interpolation from long-gap preservation, base-model training from residual
calibration, and model selection from locked testing.
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.72\linewidth]{00_framework.png}
...
128, AdamW at $10^{-3}$, early-stopping patience seven, and at most 30 epochs.

\paragraph{Common-history sensitivity analysis.}
The primary analysis uses all pre-cutoff observations, producing unequal
history lengths because BIR\_A4540 begins in 2021. A pre-specified sensitivity
runner restricts every station to a 2021 start while preserving all other
settings; its result is not included in the present evidence.

\paragraph{Metrics and paired evidence.}
MAE is primary; RMSE, sMAPE, $R^2$, bias, and fixed training-90th-percentile
event metrics are secondary. A paired bootstrap resamples 24-hour blocks of
\begin{equation}
d_{t,h}=|y_{t+h}-\hat y^{HYB}_{t,h}|-|y_{t+h}-\hat y^{TM}_{t,h}|.
\end{equation}
One thousand replicates estimate 95\% intervals; negative differences favor
the hybrid. Diebold-Mariano tests use absolute loss and Holm correction
...
Effectiveness is therefore narrow rather than global: 81 of 125 paired
bootstrap intervals favor the hybrid over TimeMixer, but none of the 25
station--horizon comparisons beats its strongest alternative. At one hour,
recent lags support a large correction; at longer horizons the gate usually
shrinks toward zero. LSTM and DLinear then benefit from stronger base
trajectories, suggesting that exogenous forecasts or a stronger backbone may
matter more than additional booster capacity.

\section{Limitations and Threats to Validity}
\label{sec:limitations}

\paragraph{Inference and construct validity.}
Overlapping windows retain serial dependence despite chronological splitting
and block inference. The locked test must not guide later tuning.
Station-level PM2.5 is not personal exposure, and the training-set 90th
percentile is an analytical event threshold rather than a health category.

\paragraph{External validity and data selection.}
Five UK stations improve on a single-site study but do not establish transfer
to unseen, industrial, or non-UK sites. Unequal histories and outages alter
the admissible sample: HP1 lacks 2016--2018, BIR\_A4540 begins in 2021, and
CHB has reduced 2024 coverage. The common-history sensitivity runner is
provided but its result is not part of the present evidence.

\paragraph{Measurement and omitted drivers.}
Instrument indicators may confound sensor response with calendar changes and
are not causal. Historical PM2.5 alone also omits meteorology, traffic,
co-pollutants, transport, and event information, limiting abrupt-episode
prediction.

\paragraph{Use boundary.}
The model is not an official warning system. Deployment should disclose
missingness, uncertainty, instrument status, update time, and abstention
conditions rather than replace monitoring or public-health guidance.

\section{Future Work}
\label{sec:future}

Future evaluation should combine leave-one-station-out transfer with
industrial and suburban sites, add forecast-available meteorology and regional
transport variables, and test change-point or regime-specific calibration.
All extensions must retain origin-safe inputs and locked testing.

\section{Conclusion}
\label{sec:conclusion}

The leakage-controlled workflow preserves long gaps and instrument regimes
while separating neural training, residual calibration, and locked testing.

\input{generated_conclusion.tex}
